...
merge_sort будет разбивать список до тех пор, пока он не доберется до одноэлементных списков.
Как только это произойдет (базовый случай рекурсии), начинает работать merge_sorted_list.
 
Для следующего примера вот что происходит:
input_list=[9, 1, 10, 2]
left=[9, 1] и right=[10, 2]
merge_sort([9, 1]) сортирует [9, 1], это назовем L1.
merge_sort([10, 2]) сортирует [10, 2], это назовем R1.
"""